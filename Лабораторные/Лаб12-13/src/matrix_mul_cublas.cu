...
template<typename T>
double run_cublas(int M, int N, int K, const T* h_A, const T* h_B, T* h_C, cublasComputeType_t computeType, cudaDataType_t dataType) {
    ...
    cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, d_B, dataType, N, d_A, dataType, K, &beta, d_C, dataType, N, computeType, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < num_configs; i++) {
        ...
        if (cfg.bytes == 2) {
            ...
            time = run_cublas<half>(M, N, K, (half*)h_A, (half*)h_B, (half*)h_C, cfg.computeType, cfg.dataType);
            ...
            time = run_cublas<float>(M, N, K, (float*)h_A, (float*)h_B, (float*)h_C, cfg.computeType, cfg.dataType);
            result = ((float*)h_C)[0];
        } else if (cfg.bytes == 1) {
            ...
            time = run_cublas<int8_t>(M, N, K, (int8_t*)h_A, (int8_t*)h_B, (int8_t*)h_C, cfg.computeType, cfg.dataType);
